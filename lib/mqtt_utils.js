...
const updateEventLogs = (sceneId, logItem) => {
    Scene.find({ gatewayId: sceneId }, (err, scenes) => { //There should be only one found ^_^
        ...
        let updatedErrorLogs = scenes[0].errorLogs ? scenes[0].errorLogs : [];
        logItem.id = updatedErrorLogs.length;
        updatedErrorLogs.push(logItem);

        console.log('To update error logs: ', updatedErrorLogs)
        Scene.updateOne({ gatewayId: sceneId }, {
            $set: {
