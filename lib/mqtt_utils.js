...
const connectToJiulongBroker = () => {
    ...
    gMqttClientJiulong.on('message', function (topic, message, packet) {
        if (topic === MQTT_TOPIC_HOST_HEARTBEAT) {
            ...
            // console.log('Jiulong host heartbeat: ', sceneInfo);
            const prevTimerId = gMapSceneHeartbeatTimer.get(sceneInfo.id);
            if (prevTimerId) { // If new message arrived before timeout, clear the timer of setting offline
            ...
            const prevTimerId = gMapSceneHeartbeatTimer.get(devicesInfo.gateway_identifier);
            if (prevTimerId) {
                clearTimeout(prevTimerId);
...
const updateDevInfo = (sceneId, inDevices) => {
    Scene.find({ gatewayId: sceneId }, (err, scenes) => { //There should be only one found ^_^
        ...
        const prevTimerId = gMapSceneHeartbeatTimer.get(sceneId);
        if (prevTimerId) { // If new message arrived before timeout, clear the timer of setting offline
            clearTimeout(prevTimerId);
        ...
        Scene.updateOne({ gatewayId: sceneId }, {
            ...
            console.log(`Update scene with new devices succeed: ${sceneId}`);
            const timerId = setTimeout(() => {
                console.log('Timeout for keeping scene online status, resetting it to be offline');
                Scene.updateOne({ gatewayId: sceneId }, {
                    $set: {
    ...
    gMqttClientLengshuo.on('message', function (topic, message, packet) {
        ...
        if (topic === MQTT_TOPIC_SALE_TABLE_STATUS) {
            ...
            if (deviceInfo.type === 'refrg_temp_hum_sensor') {
                console.log('Refrigetor temp and hum sensor: ', deviceInfo);
                const dataInfo = `TEMP1: ${deviceInfo.temperature1}, TEMP2: ${deviceInfo.temperature1}, HUMI1: ${deviceInfo.humidity1}, HUMI2: ${deviceInfo.humidity2}, Vol: ${deviceInfo.voltage}`;
