        ...
        if (topic === MQTT_TOPIC_HOST_HEARTBEAT) {
            ...
            try {
                Scene.updateOne({ gatewayId: sceneInfo.id }, {
                    ...
                    console.log(`Update scene with device heartbeat packet succeed: ${sceneInfo.id}, ${sceneInfo.scene_name}`);
            ...
            try {
                if (action === 'check') {
                    Scene.find({ gatewayId: devicesInfo.gateway_identifier }, (err, scenes) => {
                        ...
                        let mergedDevices = scenes[0].devices;
                        newDevices.forEach((newDevice, index) => {
                            const foundIndex = mergedDevices.findIndex((oldDev) => oldDev.devId === newDevice.devId);
                            if (foundIndex < 0) {
                                mergedDevices.push(newDevice);
                            }
                            else {
                                mergedDevices[foundIndex] = newDevice;
                            }
                        });
